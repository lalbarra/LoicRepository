...
public abstract class Grid {
    ...
    //Method which transforms an ArrayList<ArrayList<Integer>> grid into an int[][] grid
    ...
    private static int[][] trivialNumbers(int[][] grid){
        ...
        while (j == 1){  //Loops of Filling trivial numbers
            ...
            //Lines that only have on empty cell, get the missed trivial number
            for(int nbOfLine=0;nbOfLine<9;nbOfLine++){//We affect this idea to each line of the grid
                if (number(grid[nbOfLine],0) == 1){ //If only one cell is empty...
                    int indX = getIndex(grid[nbOfLine],0); //...the algorith search the exact position of this cell...
    ...
    private static ArrayList<Integer> twinNumbers(int[][] grid){
        ...
        for(int region=1;region<=9;region++){
            ...
            for (int indX=0;indX<8;indX++){
                if (possibilitiesInARegion.size() == 2){
    ...
    public static int[][] solve(int[][] grid2){

        int[][] grid = new int[9][9];
        //Creation of a memory of the grid
        ArrayList<ArrayList<Integer>> superMemoryGrid = changeType(grid2);
        //To recover grid using superMemoryGrid, write "
        ...
        //Memory of the Initial grid
        ArrayList<ArrayList<Integer>> memGrid = changeType(grid);
        ArrayList<Integer> twins = twinNumbers(grid);
        for(int twinHyp=0;twinHyp<=1;twinHyp++){
            if (twins.get(0)!=10){
                grid[twins.get(2)][twins.get(3)] = twins.get(twinHyp);
                grid[twins.get(4)][twins.get(5)] = twins.get(twinHyp+1-2*((twinHyp+1)/2));//We don't take 1 if twinHyp = 0, and 0 if twinHyp = 1
                grid = trivialNumbers(grid);
            }
                                    ...
                                    for(int hyp7=min(1,maxHypothesis.get(6));hyp7<=maxHypothesis.get(6);hyp7++){
                                        for(int hyp8=min(1,maxHypothesis.get(7));hyp8<=maxHypothesis.get(7);hyp8++){
                                            for(int hyp9=min(1,maxHypothesis.get(8));hyp9<=maxHypothesis.get(8);hyp9++){
                                                ...
                                                grid = changeTypeReturn(memGrid); //The hypothesis was wrong, we try an other one, by restarting the grid
                                            }
                                        }
                                    }
                                }
        ...
        grid[0][0] = 0; // We put 0 in coordinate (0,0) to know that the grid hasn't any solution
        return grid;
    }
